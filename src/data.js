...
export const projects = [
  {
    ...
    stack: ["Next.js", "Supabase", "OpenRouter", "Monaco Editor", "Tailwind CSS"],
    ...
    highlights: [
      "Architected a custom Monaco editor workspace with 4 specialized tracks (Live PR Critique, System Design, DSA Sandbox, Resume Grill) tailored dynamically to candidate profiles",
      "Built a custom Edge runtime stream reader decoupling real-time OpenRouter LLM streaming from Supabase DB logging, eliminating UI lockups and ensuring low latency",
      "Implemented strict scorecard evaluation (analyzing word count and submissions against rubrics) and background session filters that automatically purge stale transcripts",
    ],
  ...
  },
  {
    ...
    highlights: [
      "Architected a horizontally scalable URL shortener on AWS EC2 using Nginx round-robin load balancing across stateless FastAPI instances and PostgreSQL primary-replica streaming replication",
      "Achieved sub-millisecond redirect latency (p99 < 1.5ms) using Redis cache-aside, with atomic sliding-window rate limiting and secure bijective 64-bit Feistel cipher short codes",
      "Engineered an asynchronous analytics pipeline utilizing RabbitMQ for fire-and-forget event publishing with competing consumers, exponential backoff, and dead-letter queues",
    ],
